...
At the end of the run, before handing back, check the log told the truth. Read this run's pi session file (`/session` prints the path; sessions live under `~/.pi/agent/sessions/` per working directory). Don't glob across other workspaces' session dirs; that reads unrelated private sessions. Walk the log against what actually happened:
...
Before handing back, you must dispatch a worker on a different agent family from the one that did the work. Self-review is not a substitute; the point is fresh eyes you cannot bring yourself. The worker reads the audit trail and the run's session, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
