...
4. Verify each component against its baseline via image diff on the matching surface (ego-browser for web UIs, the real app otherwise). A nonzero diff is a fail; investigate the pixel delta, don't wave it through. `/loop` per component until the diff is zero.
