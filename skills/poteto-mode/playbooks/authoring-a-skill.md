...
1. Author per pi's skill format: a directory with a `SKILL.md`, frontmatter `name` + `description` (both required, description drives triggering), relative paths for scripts and references. pi discovers skills from `~/.pi/agent/skills/`, project `.pi/skills/`, or package `skills/` directories. Keep the description specific; it is the only always-loaded line.
