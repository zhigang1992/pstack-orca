...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Workers are dispatched, followed up, and drained only through Orca orchestration (`task-create`, `worker-start`, `check --wait`); the Run is the program's durable namespace and inbox. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out, to conserve context. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer: each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, dispatches its own workers and verifiers (Orca nested worker depth is a setting, `Settings -> Orchestration -> Nested worker depth`, default 1; raise it to 2 before a sub-coordinator may dispatch). Rolls up aggregates at wave boundaries; never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window; never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Default placement is a fresh Orca worktree (`--worktree new-top-level --name <unit> --setup run`); keep a worker in the coordinator's worktree (`--worktree current`) only when the task needs this checkout: runtime verification against local state, reading local sessions under `~/.pi/agent/sessions/`, simulators, or auth that exists only here. Use `--on <environment>` for work that belongs on another paired machine. Workers in other worktrees cannot read the coordinator's store, so their briefs inline what they need or point at repo paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different agent family from its worker.
...
- `inbox/` holds completion pointers. `gates.md` parks human gates (question, options, default on no answer) so a completion flood cannot wipe the pending-question state.
...
VERIFY       exact commands or the surface-driver path (ego-browser, the real CLI), plus known gotchas
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape; a 4KB scaffold around a two-line edit costs more to write and obey than the edit. Same-worktree workers may reference the standing-orders file by store path; verbatim paste is for workers in other worktrees and every re-dispatch.

A sub-coordinator brief adds its track boundary and unit list, its dispatch budget with the new-worktree default and the same-worktree exception list, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line; plus track status and frontier delta).
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (a rolling `orca orchestration check --wait` while you are in the session, or an Orca automation re-prompting this terminal, with a long heartbeat fallback), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. Restacks run in a dedicated worker (optionally `--on <environment>`); a local restack at this scale takes the laptop down.
...
- Never re-prompt a worker to check on it; a fresh prompt to an idle worker restarts it. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, `orca orchestration worker-show --dispatch <id> --json`, and bounded `worker-read`. Terminal activity is not liveness of the task.
...
- After an Orca or machine restart: the coordinator's workers may be dead, but pushed branches and PRs are not. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach in-flight work by PR and branch rather than dispatch id, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone.
