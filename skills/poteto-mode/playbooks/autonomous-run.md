...
**You own the exit condition. Define done, then drive to it without stopping.** For "going to bed" / "run until done" / "keep at it until X".
...
2. Pick the wake mechanism. Inside Orca, a rolling `orca orchestration check --wait` loop covers supervised workers; for time- or event-based wakes while you are away, create an Orca automation (`orca automations create --trigger <preset|cron> --prompt <text> --provider <agent>`) that re-prompts this terminal or a fresh one. An event to watch (CI, a merge, a ref advancing) gets a watcher worker that reports on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.
