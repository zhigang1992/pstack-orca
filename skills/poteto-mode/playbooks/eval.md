...
- The judge can know it's judging but sees outputs by sanitized label only, never by agent or model name.
...
4. **Spawn N parallel candidates** on different agent families per the **arena** skill's Phase B. Each works in its own sanitized dir; same prompt to each.
5. **Spawn one blinded judge** on a different agent family per the **arena**'s Phase C. Judge sees outputs by sanitized label and the rubric, never an agent or model name.
6. **Verify the chain from sessions, not self-report.** Read each candidate worker's session through `orca orchestration worker-read --dispatch <id> --json` (or its pi session JSONL under `~/.pi/agent/sessions/`)
