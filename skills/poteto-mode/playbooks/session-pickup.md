...
**You own the resume point. Read the prior trail, don't redo it.** For "take over this", "resume this conversation", "continue from <transcript path>", "you're taking over", "pick up where X left off", an Orca worker handoff, or a pushed branch you're meant to continue.
...
1. Locate the prior trail. A pi session file under the current workspace's directory in `~/.pi/agent/sessions/` (`/session` prints the active one; do not glob across other workspaces' session dirs, that crosses workspace boundaries and reads private sessions from unrelated projects), a live Orca worker readable through `orca orchestration worker-read --dispatch <id> --json`, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long session in a worker and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
