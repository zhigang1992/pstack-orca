Synthesize three reviewers' findings from the active transcript into skill edits, backlog items, or rejections. Do not modify files; the parent applies the Accepted list after user approval. Use any connector available in your environment (CLIs like `gh` or `lark-cli`, installed skills, MCP tools if present) to verify a finding (e.g. ticket, observability trace, chat thread).
...
- Skill-was-used: only accept findings that route to a skill, tool, or connector the parent actually invoked in the transcript. If the skill wasn't used but should have been, route to `tune description: <skill path>` so it triggers next time. If neither, reject as `skill-not-used`.
