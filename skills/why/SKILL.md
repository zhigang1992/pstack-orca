---
name: why
description: "Use for 'why does X work this way', 'why we picked Y', design rationale, regressions, postmortems, or data-backed thresholds. Discovers available evidence connectors (skills, CLIs, MCP servers) and queries each category (source control, issue tracker, long-form docs, real-time chat, infrastructure observability, error tracking, product analytics warehouse) in parallel, then returns a cited read on decisions and tradeoffs. Use how for runtime behavior."
---
...
Before dispatching investigators, enumerate the evidence connectors available in this environment. In the pi + Orca setup there is no MCP registry to inspect; a connector is anything a worker can drive:

- Installed skills (the pi skill list). In this environment the `lark-*` skills cover real-time team chat, long-form docs, calendars, tasks, approval records, and meeting notes through `lark-cli`.
- CLIs on PATH: `gh` (source control and GitHub Issues), `lark-cli`, plus any observability or error-tracking CLIs the team has installed.
- MCP servers, if your harness exposes them to workers.

Map each available connector to one evidence category:
...
Source control is always available through git and `gh`. For the other six, classify using the connector's name and surface (skill description, CLI help, MCP tool names). If a connector could fit more than one category, choose the one matching its primary evidence. Record ambiguous cases in the coverage map.
...
Create one Task per investigator and start all workers before waiting, so they run concurrently. One investigator per category lets each specialize in one connector's query vocabulary and result shape. Don't ask one worker to cover multiple connectors.

Worker launch (each):
- agent: your configured why-investigators worker (default `grok`)
- brief: investigate and report; no writes to the repo. That's a posture, not a sandbox.
- placement: `--worktree current`

Each investigator gets:
1. The base prompt from `references/investigator-prompt.md`
2. The category playbook `references/sources/<source>.md` for the selected connector, adapted from the examples in `references/source-playbook.md`
...
Spawn one investigator per category that has a matching connector. Each owns exactly one connector.
...
Dispatch one synthesizer worker:

- agent: your configured why-synthesizer worker (default `claude`)
- placement: `--worktree current`
- brief: synthesize and report. The synthesizer's quality check spot-verifies citations, which can require re-querying a connector, so its brief names the connectors the investigators used.
...
**Sources Consulted**. One line per investigator, including the ones that returned nothing. The reader should see at a glance (a) which connectors were queried, (b) which came back empty, and (c) which were skipped and why. This coverage map lets the user judge breadth and redirect if something obvious was missed.
...
- Real-time team chat (Slack): skipped. No matching connector available in this environment. Gap: conversational record not searched.
...
- **Collapsing investigators into one worker**. Each connector has its own query vocabulary, result shape, and pitfalls; pooling them dilutes specialization and makes coverage harder to reason about. Always one investigator per category.
