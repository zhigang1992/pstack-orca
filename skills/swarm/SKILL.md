...
4. Pick the worker from `swarm workers` in the pstack config (project `.pstack-models.md`, then user `~/.pi/agent/pstack-models.md`) when present. Otherwise use `grok`. For an agent race, name each arm's agent up front.
