...
- A fix-phase code worker may edit only when its environment provably excludes chat credentials and every chat write action. Otherwise the coordinator edits.
- Every child brief must explicitly forbid every chat write action (`lark-cli` message sends, `SendSlackMessage`, `PostToSlack`, `chat.postMessage`, and all others).
- Never give a child a chat token, posting instructions, source coordinates for posting, or permission to report externally.
- If a child needs chat write access to run, do not dispatch it.
...
Prefer the configured chat adapter's own actions. Use `BENNY_SLACK_BOT_TOKEN` only when the user configured it for a narrow missing capability such as editing this one status message. Never expose the token to a worker.
...
The coordinator owns every channel post, the final diff review, commits, and the pull request.
...
A tightly scoped code edit may be delegated during this phase only when tool isolation removes chat credentials and every chat write action from that worker. Its prompt must still carry the explicit Slack-write ban. The coordinator reviews the edit and runs or verifies the required tests. If tool isolation is uncertain, keep the edit in the coordinator.
